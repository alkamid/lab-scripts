...
class matplotLIV():
    ...
    def __init__(self, BaseFilename, temperatures, length=None, width=None, area=None, title = '', ylim=None):
        ...
        filenames = [("%s_%dK.txt" % (self.BaseFilename, temp), temp) for temp in self.temperatures]
    ...
    def plot(self):
        ...
        for i, (datafile, label) in enumerate(self.rawData):
            ...
            ax1.plot( datafile[:,0], datafile[:,1], color=self.colors[i], label='%dK' % label)
            ax2.plot( datafile[:,0], datafile[:,2], color=self.colors[i], label='%dK' % label, linewidth=2)
        ...
        fig.suptitle(self.title, y=0.98, weight='bold')
